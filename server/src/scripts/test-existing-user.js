...
async function testWithUser(userId) {
  logSection('Starting Supabase Test with Existing User');
  ...
  try {
    ...
    // Create an anon client (should not be able to access data)
    ...
    if (anonError) {
      console.log('✅ RLS working: Anonymous client cannot access profiles');
    } else if (anonData && anonData.length > 0) {
      console.log('⚠️ RLS issue: Anonymous client can access profiles!');
    }
    ...
    console.log('\nThe Supabase database is correctly set up and functioning!');
